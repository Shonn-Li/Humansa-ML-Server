...
def test_proxy_direct():
    ...
    if not proxy_username or not proxy_password:
        logger.error(
            "❌ WEBSHARE_PROXY_USERNAME or WEBSHARE_PROXY_PASSWORD not found in .env file!")
        logger.error("These are NOT the API key!")
        logger.error(
            "Go to your Webshare dashboard -> Proxy Settings to find these values")
        return False
    ...
    logger.info(
        "🌐 Configuring WebshareProxyConfig for rotating residential proxies")
    ...
    try:
        logger.info(
            "🔄 Initializing YouTubeTranscriptApi with WebshareProxyConfig...")
        ...
        logger.info(
            f"   Total duration: {sum(s['duration'] for s in transcript):.2f} seconds")
        ...
        if "404" in str(e) or "Unable to connect to proxy" in str(e):
            logger.error(
                "🔴 Proxy connection failed - Invalid credentials or configuration")
            logger.error("Please verify:")
            logger.error(
                "1. You have an active Webshare 'Residential' proxy package (not free tier)")
            logger.error("2. Your proxy username and password are correct")
            logger.error(
                "3. Your Webshare account is active and has available bandwidth")
        ...
        try:
            ...
            logger.warning(
                "⚠️ Direct access works but proxy failed - check proxy configuration")
            logger.warning(
                "This confirms the issue is with the Webshare proxy setup")
        except Exception as direct_e:
            logger.error(f"❌ Direct access also failed: {str(direct_e)}")
            logger.info(
                "Both proxy and direct access failed - might be rate limited")
