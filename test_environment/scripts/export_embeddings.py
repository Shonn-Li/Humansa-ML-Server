#!/usr/bin/env python3
"""
Export embeddings from production database for test data
...
def generate_sql(embeddings):
    ...
    # Header
    sql_lines.append("-- Embeddings for test data")
    ...
    for i in range(0, len(embeddings), batch_size):
        ...
        sql_lines.append("  source, url, metadata, chunk_tsv, last_updated")
        sql_lines.append(")")
        ...
        for emb in batch:
            ...
            tsv_val = f"'{emb['chunk_tsv']}'::tsvector" if emb['chunk_tsv'] else 'NULL'
            
            values.append(
                f"  ({emb['type_id']}, '{emb['type']}'::varchar(50), {emb['section_id']}, "
                f"{embedding_val}, '{chunk_text}'::text, "
...
def main():
    print("Exporting embeddings from production database...")
    ...
    try:
        ...
        # Save to file
        output_path = '../sql/04_embeddings.sql'
        with open(output_path, 'w') as f:
            f.write(sql)
        
        print(f"\nSQL saved to: {output_path}")
        print(f"File size: {len(sql) / 1024 / 1024:.2f} MB")
