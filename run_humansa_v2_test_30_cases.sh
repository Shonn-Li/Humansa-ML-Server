...
echo "============================================"
echo "HUMANSA AI AGENT V2 - 30 TEST CASES"
echo "============================================"
...
python3 -c "from mem0 import Memory; print('✅ Mem0 package is installed')" || {
    echo -e "${RED}❌ Mem0 package not found. Install with: pip install mem0ai${NC}"
    exit 1
}
...
python3 -m src.main --port 6001 > server_v2_test_30.log 2>&1 &
SERVER_PID=$!
...
if ! curl -s http://localhost:6001/v2/humansa/health > /dev/null 2>&1; then
    echo -e "\n${RED}❌ Server failed to start. Check server_v2_test_30.log for errors${NC}"
    kill $SERVER_PID 2>/dev/null || true
    exit 1
fi

# Step 5: Run the 30 test cases
...
tail -n 50 server_v2_test_30.log
...
echo "- Test results: humansa_v2_test_results_30cases_*.json"
