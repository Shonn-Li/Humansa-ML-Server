...
class TextTruncator:
    ...
    def __init__(self, model: str = "text-embedding-3-small"):
        ...
        logger.info(
            f"TextTruncator initialized for {model} (limit: {self.max_tokens} tokens)")
    ...
    def truncate_text(self, text: str, max_tokens: Optional[int] = None,
        ...
        logger.info(
            f"Truncating text: {original_token_count} tokens -> {limit} tokens")
        ...
        while left < right:
            ...
            if preserve_sentences and mid < len(text):
                candidate_text = self._truncate_to_sentence_boundary(
                    candidate_text)
        ...
        if not best_text:
            logger.warning(
                "Binary search failed, using character-based fallback")
            best_text, best_tokens = self._character_based_truncation(
                text, limit)

        logger.info(
            f"Text truncated: {original_token_count} -> {best_tokens} tokens ({len(text)} -> {len(best_text)} chars)")
    ...
    def truncate_texts_batch(self, texts: List[str], max_tokens_per_text: Optional[int] = None,
        ...
        total_limit = max_total_tokens if max_total_tokens is not None else per_text_limit * \
            len(texts)
        ...
        for text in texts:
            truncated_text, token_count, _ = self.truncate_text(
                text, per_text_limit)

            # Check if adding this text would exceed total limit
            if total_tokens + token_count > total_limit:
                logger.warning(
                    f"Batch total token limit would be exceeded, stopping at {len(truncated_texts)} texts")
                break
        ...
        logger.info(
            f"Batch truncation: {len(texts)} -> {len(truncated_texts)} texts, {total_tokens} total tokens")
