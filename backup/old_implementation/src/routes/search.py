...
async def keyword_search_endpoint():
    ...
    try:
        ...
        # Validate request
        is_valid, error_msg, params = search_service.validate_search_request(
            request_data)
        if not is_valid:
            return jsonify({"error": error_msg}), 400

        logger.info(
            f"Keyword search for user {params['user_id']}: '{params['query']}' (limit: {params['limit']})")
...
async def advanced_search_endpoint():
    ...
    try:
        ...
        logger.info(
            f"Advanced search for user {params['user_id']}: '{params['query']}' (source: {params['source_filter']}, limit: {params['limit']})")
