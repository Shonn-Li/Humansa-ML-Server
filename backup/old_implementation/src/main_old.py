...
def register_original_endpoints(app):
    ...
    # Import link analyzer from new independent link module
    try:
        from src.link.link_analyzer import analyze_link
        LINK_ANALYZER_AVAILABLE = True
    except ImportError:
        logger.warning("Link analyzer not available")
        LINK_ANALYZER_AVAILABLE = False

    # Import enhanced chat bot from backup location for V1 endpoints compatibility
    try:
        from backup.old_implementation.src.ai_chat_bot.enhanced_chat_bot import (
            EnhancedChatBot,
