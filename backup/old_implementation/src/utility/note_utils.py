...
from src.utility.postgres import get_embeddings, save_embeddings, get_note_text, get_embeddings_v2, save_embeddings_v2
...
def get_most_related_notes_pgvector(
    ...
    if notes_without_embeddings:
        # Sanity check to prevent mass re-embedding due to bugs
        if len(notes_without_embeddings) > 100:
            logger.error(
                f"⚠️  MASS RE-EMBEDDING DETECTED: {len(notes_without_embeddings)} notes without embeddings!")
            logger.error(
                "This is likely a bug. Limiting to first 10 notes to prevent API cost explosion.")
            notes_without_embeddings = notes_without_embeddings[:10]
        ...
        for note_id in notes_without_embeddings:
            try:
                ...
                # If note has no content, mark it to skip future embedding attempts
                if "No content found" in str(e):
                    logger.info(
                        f"Note {note_id} has no content, marking skipEmbedding=true")
                    mark_note_skip_embedding(note_id)
                else:
                    logger.error(
...
def get_most_related_chunks_pgvector(
    ...
    for note_id, section_id, similarity, chunk_text, source in chunk_results:
        ...
        if chunk_text and chunk_text.strip():
            ...
            logger.info(
                f"Found chunk from note {note_id} section {section_id} (similarity: {similarity:.3f}, source: {source})")
        else:
            logger.warning(
                f"Empty chunk text for note {note_id} section {section_id}")
...
def create_and_save_embeddings_separate(note_id: int):
    ...
    logger.info(
        f"Note {note_id}: Processing AI content ({len(ai_content)} chars) and user content ({len(user_content)} chars) separately")

    # Split using optimized chunk size for better retrieval
    splitter = SentenceSplitter(chunk_size=1024, chunk_overlap=100)

    # List of (embedding_vector, chunk_text, source) tuples
    embeddings_data = []
    ...
    if user_content.strip():
        ...
        for chunk in user_chunks:
            all_chunks_for_embedding.append(chunk)
            # source = note_type for user content
    ...
    for i in range(0, len(all_chunks_for_embedding), batch_size):
        ...
        logger.info(
            f"Processing batch {i//batch_size + 1} with {len(batch_chunks)} chunks")
        batch_embeddings = embedder._get_text_embeddings(batch_chunks)
        all_embeddings.extend(batch_embeddings)
...
def mark_note_skip_embedding(note_id: int):
    """Mark a note to skip embedding when it has no embeddable content"""
    ...
    try:
        ...
        with conn.cursor() as cursor:
            cursor.execute(
                'UPDATE note_v1 SET "skipEmbedding" = true, "updateDate" = NOW() WHERE id = %s',
                (note_id,)
            )
            conn.commit()
            logger.info(f"Marked note {note_id} as skipEmbedding=true")
    except Exception as e:
        logger.error(f"Failed to mark note {note_id} as skipEmbedding: {e}")
    finally:
        if conn:
            conn.close()
